---
title: "When Good Features Kill Each Other: Cannibalization and the Illusion of Growth"
...
date: 2026-08-24
categories: [experimentation, causal inference, marketing analytics, product analytics, R]
---
...
```{mermaid}
%%| fig-cap: "Any observed gain splits into three sources. Only creation is growth. Substitution and temporal displacement together make up cannibalization."
flowchart LR
...
```{mermaid}
%%| fig-cap: "The reported gross lift decomposes into the net value the business is paid for plus the cannibalization term C. Everything to the right of the green node is the uncounted loss."
flowchart LR
...
::: {.callout-note title="A brief history: from Kotler to TikTok"}
...
::: {.callout-tip title="The four types, at a glance"}
...
```{mermaid}
%%| fig-cap: "The four types ordered left to right by the time-scale on which substitution becomes visible, from the fastest (product, within a session) to the slowest (temporal, weeks to months). The measurement window has to match the type."
flowchart LR
...
#| fig-width: 7.5
#| fig-height: 3.8
#| fig-cap: "Temporal cannibalization. Demand peaks during the promotion window then falls below the baseline of 50 for several weeks before recovering. The gross lift counts only the peak. The net lift subtracts the valley that follows."
...
demand_curve <- tibble(
  t      = 1:7,
  week   = c("pre", "promo 1", "promo 2", "post 1", "post 2", "post 3", "recover"),
  demand = c(50, 90, 88, 22, 31, 42, 50)
)

ggplot(demand_curve, aes(x = t, y = demand)) +
  annotate("rect", xmin = 1.5, xmax = 3.5, ymin = 0, ymax = 100,
           fill = "#2C7FB8", alpha = 0.10) +
  annotate("rect", xmin = 3.5, xmax = 6.5, ymin = 0, ymax = 50,
           fill = "#D95F0E", alpha = 0.08) +
  geom_hline(yintercept = 50, linetype = "dashed", colour = "#8a97a4") +
  geom_line(colour = "#D95F0E", linewidth = 1.5) +
  geom_point(colour = "#D95F0E", size = 2.8) +
  annotate("text", x = 2.5, y = 97, label = "promotion window",
           colour = "#2C7FB8", size = 3.4) +
  annotate("text", x = 2.5, y = 82, label = "peak (gross lift)",
           colour = "#b5751a", size = 3.5, fontface = "bold") +
  annotate("text", x = 5.0, y = 20, label = "post-window valley",
           colour = "#b23a2b", size = 3.5, fontface = "bold") +
  annotate("text", x = 6.7, y = 55, label = "baseline",
           colour = "#8a97a4", size = 3.2) +
  scale_x_continuous(breaks = demand_curve$t, labels = demand_curve$week) +
  scale_y_continuous(limits = c(0, 100)) +
  labs(x = NULL, y = "Demand", title = "Demand around a promotion") +
...
```{mermaid}
%%| fig-cap: "The local metric is a subset of the total-metric scope. A/B tests instrument the inner box (Y-L, the surface the team owns). The business is paid for the whole outer box (Y-T, what the income statement records), which also contains the surfaces the intervention can displace."
flowchart TB
...
::: {.callout-note title="The estimand comes first"}
...
::: {.callout-warning title="SUTVA can hold locally and fail totally"}
...
::: {.callout-tip title="Which approach for which type"}
...
: Which approach for which type. ✓ natural fit, ○ adaptable with care, — not the right tool. This mapping is my working judgement rather than a settled consensus, and an experienced practitioner will reasonably move a cell or two. The load-bearing claim is only that no single approach covers all four types. {.striped tbl-colwidths="[40,15,15,15,15]"}
...
::: {.callout-warning title="An uncalibrated cross-elasticity is a hypothesis, not a finding"}
...
The two scenarios now sit side by side in a single data frame. The same 20-unit lift is built into the local metric in both. The total metric moves by 20 in scenario A and by 0 in scenario B. Everything else, the baseline levels, the noise draws, and the treatment assignment, is shared.
...
::: {.callout-important title="The A/B test cannot see the difference"}
...
```{mermaid}
%%| fig-cap: "The review gate. The five questions run in sequence, and a failure at any single step means the reported gain is partly or wholly displacement until proven otherwise."
flowchart TD
